...
L_wt = 25  # Life of turbine [years]

##Aelos V 10 kW wind turbine parameters
pr = 0.01  # rated power [MW] (10 kW)
p_max = 0.012  # maximum output power [MW] (12 kW)

h = 6.0  # hub height / rotor height [m]
D_rotor = 5.5  # rotor diameter [m]
R_rotor = D_rotor / 2

vci = 2.5  # cut-in wind speed [m/s]
vr = 11.0  # rated wind speed [m/s]
vco = 52.5  # survival wind speed [m/s]

omega_max = 260  # maximum rotational speed [rpm]
eta_gen = 0.96  # generator efficiency [-]

mass_wt = 680  # turbine weight [kg]

##Environmental parameters (unchanged)
z0_BHP = 0.03  # surface roughness length [m]
z0_IGO = 0.80  # surface roughness length [m]

rho_0 = 1.225  # air density at standard condition [kg/m3]
R = 287.05  # ideal gas constant [J/kg.K]
T0 = 273.15  # reference temperature [K]

##Fixed unit cost parameters of wind turbine (scaled / generic – TO BE REVIEWED)
c_rt = 470  # rotor cost [AUD/kW]
c_nac = 768  # nacelle cost [AUD/kW]
c_twr = 306  # tower cost [AUD/kW]
c_eng = 34.5  # engineering cost [AUD/kW]
c_pm = 15  # project management [AUD/kW]
c_fdn = 112.5  # foundation [AUD/kW]
c_ssf = 60  # site access & facilities [AUD/kW]
c_inst = 61.5  # installation [AUD/kW]
c_ei = 198  # electrical infrastructure [AUD/kW]
c_ctfn = 34.5  # construction financing [AUD/kW]
c_cont = 135  # contingency [AUD/kW]

c_uWTOaM = 60  # operation & maintenance cost per year [AUD/kW]

##Constant input parameters of Wind Turbine Cost Model
beta_OaM = 0.04  # O&M cost factor
beta_ins = 0.01  # insurance cost factor

##Life cycle emission of Wind Turbine
em_WT = 0.015  # life cycle emission [kg CO2-e/kWh]
...
Li_cost_proj_E = pd.read_excel(
    "input_data//Cost Projection Values.xlsx",
    sheet_name="Energy Components",
    usecols="M:Q",
    nrows=29,
)
Li_cost_proj_P = pd.read_excel(
    "input_data//Cost Projection Values.xlsx",
    sheet_name="Power Components",
    usecols="M:Q",
    nrows=29,
)
